...
import {type ImageKitInputProps} from '../util/types'

export interface Props extends Pick<ImageKitInputProps, 'schemaType'> {
...
function ErrorBoundaryCard(props: Props) {
  ...
  const {ErrorBoundary, didCatch, error, reset} = useErrorBoundary({
    onDidCatch: (err, errorInfo) => {
      if (err.stack) {
        // Error stack information available for debugging
      }
      if (errorInfo?.componentStack) {
        // Component stack information available for debugging
      }
      pushToast({
        status: 'error',
