...
export default function DeleteDialog({
  ...
  >('checkingReferences')
  const [deleteOnImageKit, setDeleteOnImageKit] = useState(true)
  ...
  async function confirmDelete() {
    ...
    const worked = await deleteAsset({client, asset, deleteOnImageKit})
    if (worked === true) {
      toast.push({title: 'Successfully deleted video', status: 'success'})
      succeededDeleting()
    } else if (worked === 'failed-imagekit') {
      toast.push({
        title: 'Video document deleted',
        description: "But it wasn't deleted in ImageKit",
        status: 'warning',
      })
      succeededDeleting()
    } else {
            ...
            <>
              ...
              <Stack space={4} marginY={4}>
                <Flex align="center" as="label">
                  <Checkbox
                    checked={deleteOnImageKit}
                    onChange={() => setDeleteOnImageKit((prev: boolean) => !prev)}
                  />
                  <Text style={{margin: '0 10px'}}>Delete asset on ImageKit</Text>
                </Flex>
                <Flex align="center" as="label">
